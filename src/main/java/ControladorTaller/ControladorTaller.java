package ControladorTaller;
	...
	public class ControladorTaller {
	    ...
	    private void login() {
			...
			if (usuarioDB != null && PasswordUtils.verifyPassword(password, usuarioDB.getPassword())) {
	    ...
	    private void consultarTodasReparaciones() {
	        System.out.println("\n--- CONSULTAR TODAS LAS REPARACIONES ---");
	        
	        try {
	            
	            ArrayList<Reparacion> reparaciones = (ArrayList<Reparacion>) reparacionDAO.findall(); 

	            if (reparaciones.isEmpty()) {
	                System.out.println("Actualmente no hay reparaciones registradas.");
	                return;
	            }
	    ...
	    private void registrarNuevaReparacion() {
	        ...
	        try {
	            ...
	            coste = Double.parseDouble(sc.nextLine());
	        } catch (NumberFormatException e) {
	    ...
	    private void modificarDatosReparacion(boolean soloPropias) {
	        System.out.println("\n--- MODIFICAR DESCRIPCIÓN Y COSTE (CU4 Modificado) ---");

	        ArrayList<Reparacion> reparaciones;
	        
	        try {
	            if (soloPropias) {
	                reparaciones = reparacionDAO.findByUsuario_id(usuarioActual.getId());
	            } else {
	                reparaciones = reparacionDAO.findall(); 
	            }
	            ...
	            System.out.println("Reparaciones disponibles:");
	            consultarTodasReparaciones();  
	            
	            System.out.print("Ingrese ID de la reparación a modificar: ");
	            int idReparacion = Integer.parseInt(sc.nextLine());
	    ...
	    private void verEstadisticas() {
	        System.out.println("\n--- ESTADÍSTICAS DEL TALLER (CU6 Modificado) ---");
	    ...
	    private int mostrarMenuAdministrador() {
	        ...
	        System.out.println("5. Ver Estadísticas (CU6 Modificado)");
	        System.out.println("0. Cerrar Sesión");
	        System.out.print("Seleccione una opción: ");
